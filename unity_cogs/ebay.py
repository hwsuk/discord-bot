...
class Ebay(commands.Cog):
    ...
    async def check(self, ctx, *, search_term: str):
        ...
        async with ctx.channel.typing():
            ...
            embed_details = {
                "title": f"Results for {filtered_term}",
    ...
    def make_embed(self, data: dict) -> discord.Embed:
        ...
        embed.add_field(
            name="Range",
            value=f"£{as_price(data['range'][0])} - £{as_price(data['range'][1])}",
            inline=True,
        )
    ...
    def parse_date(self, product) -> datetime.datetime:
        ...
        month_regex = re.compile(r"\d{2}-(\w*)\W\d{2}:\d{2}")
